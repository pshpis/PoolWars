...
const MenuElement = ({children, currentSection, setCurrentSection}) => {
    ...
    return <Box lineHeight="38px" height="38px" width="100%"
                ...
                onMouseLeave={() => setHover(false)}>{children}</Box>;
}

export const Menu = ({currentSection, setCurrentSection}) => {

    let menuElements = sections.map((section) => <MenuElement currentSection={currentSection} setCurrentSection={setCurrentSection} key={section}>{section}</MenuElement>);
    return (
